...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            /* 注册会员邮件欢迎 */$tpl = get_mail_template('send_reg');$smarty->assign('shop_name', $_CFG['shop_name']);$smarty->assign('send_date', date($_CFG['time_format']));$smarty->assign('user_name',$username);$content = $smarty->fetch('str:' . $tpl['template_content']);send_mail($_CFG['shop_name'], $email, $tpl['template_subject'], $content, $tpl['is_html']);
            /* 判断是否需要自动发送注册邮件 */
